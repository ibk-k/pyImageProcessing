...
/**
 ...
 @param p:	file path
 @return		pointer is end of file path except for extension
 */
...
/**
 @brief	show information about argument
 @details	argument are frame index and session index. enable to abbreviate session index
 @param executefile:	execute file name
 @param targetkind:		target data kind
 @return 2
 */
...
/**
 @brief	show information about argument
 @details	argument are frame index and session index. enable to abbreviate both
 @param executefile:	execute file name
 @param targetkind:		target data kind
 @return 2
 */
...
/**
 ...
 @return 2
 */
int usage(int argflag_frame, int argflag_session, const char* executefile, const char* targetkind)
{
	if ((argflag_frame & ARGFLAG_ENABLE)
		&& (argflag_session & ARGFLAG_ENABLE))
	{
		if ((argflag_frame & ARGFLAG_ABBR)
			&& (argflag_session & ARGFLAG_ABBR))
		...
		if (argflag_session & ARGFLAG_ENABLE)
		{
			if (argflag_session & ARGFLAG_ABBR)
				printf("unknown argument format!\n");
			else
	...
	*/
	int		interpret_option(const char* arg)
	{
		if (*arg == '@')
		{
			...
			if (!get_number(++arg, a))
			{
				printf("@ is prefix for specifying session and following word has to be number.\n");
				return -1;
			}
		...
		{
			// show uage
			return usage(m_argflag_frame, m_argflag_session, m_executefile, m_targetkind);
		}
		...
		// check frame number
		{
			...
			if (!get_number(arg, a))
			{
				...
				{
					if (m_iFrame == NOTSET_FRAMEINDEX && (m_argflag_frame & ARGFLAG_ABBR) == 0)
					{
				...
				if (m_argflag_session & ARGFLAG_ENABLE)
				{
					if (m_iSession == NOTSET_SESSIONINDEX && (m_argflag_session & ARGFLAG_ABBR) == 0)
					{
